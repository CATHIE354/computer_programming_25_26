...
class IrisRuleClassifier:
    ...
    def update_result_counts(self, correct, wrong, total, y_pred_list, y_pred, y_true):
        ...
        #Always increase total by 1 (even when the prediction is wrong!)
        #Append y_pred to y_pred_list
        #Return the tuple: (correct, wrong, total, y_pred_list)
...
def main():
    ...
    sample = {"petal_length": 1.4, "species": "setosa"}
    prediction = classifier.compute_threshold_prediction(sample)
    print("Prediction:", prediction) # should print: setosa
