...
def compute_threshold_prediction(flower):
    ...
    if flower[feature_name] < threshold:
...
def run_prediction_loop(dataset):
    ...
    for flower in dataset:
        print("\nProcessing flower with id:", flower["id"])

        # Task 5: Predict the class using petal length
        y_pred = compute_threshold_prediction(flower)
        y_true = derive_true_label(flower)
